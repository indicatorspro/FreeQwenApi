 ...
 *   FREEQWEN_API_URL  — proxy base URL (default: http://127.0.0.1:3264/api)
 */
...
Object.keys(process.env).sort().forEach(key => {
    if (key.match(/CONV|TEMP|DIR|SESSION|AION|WORK/i)) {
        log(`  ${key}=${process.env[key]}`);
    }
});

// ─── Retry Configuration (aligned with proxy MAX_RETRY_COUNT) ────────────────

const MAX_RETRIES = 3;
const RETRY_DELAY_MS = 2000;
const POLL_CONSECUTIVE_FAILURE_WARN = 3; // warn after N consecutive polling failures
...
const TOOLS = [
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                model: {
                    type: 'string',
                    description: `Model to use for image generation. Default: ${DEFAULT_MODEL}. Options: qwen-image-max, qwen-image-plus, qwen-image, wan2.6-t2i, wan2.5-t2i-preview, wan2.2-t2i-flash`
                },
                save_dir: {
                    type: 'string',
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                model: {
                    type: 'string',
                    description: `Model to use for video generation. Default: ${DEFAULT_MODEL}. Options: wan2.6-t2v, wan2.6-t2v-preview, wan2.2-t2v-flash`
                },
                save_dir: {
                    type: 'string',
    ...
    switch (method) {
        case 'initialize':
            sendResponse(id, {
                ...
                capabilities: { tools: {} },
                serverInfo: { name: 'Qwen Media Creator', version: '1.0.0' }
            });
            break;
...
async function generateVideo({ prompt, size = '16:9', model = DEFAULT_MODEL, save_dir = null, filename = null }) {
    ...
    const createRes = await fetchWithRetry(`${API_URL}/videos/generations`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt, size, model, wait: false })
    ...
    if (!taskId) {
        if (createData.video_url) {
            ...
            let localPath = null;
            try {
                localPath = await downloadAndSave(createData.video_url, save_dir, filename, 'mp4');
                log(`✅ Saved to: ${localPath}`);
            } catch (e) {
